...
For the core of our analysis, we employed three primary Spatial interpolation techniques: Inverse Distance Weighting (IDW), Ordinary Kriging (OK), and Universal Kriging. These methods, particularly OK and Universal Kriging, offer sophisticated approaches to spatial interpolation, considering both spatial autocorrelation and a deterministic trend. We meticulously fine-tuned these kriging models, fitting variogram models and validating their performance to ensure accurate predictions.
...
## Prediction with Universal kriging
```{r,warning=FALSE,message=FALSE}
...
In this analysis of zinc concentrations in the Meuse river region, we employed various spatial interpolation techniques to predict zinc levels across the area. Our exploration began with an examination of the Meuse dataset, revealing spatial patterns in zinc distribution. We then conducted Inverse Distance Weighting (IDW), Ordinary Kriging (OK), and Universal Kriging to interpolate zinc concentrations.

Through exploratory data analysis (EDA) and variogram analysis, we identified spatial relationships and fitted appropriate models for kriging. The resulting kriging predictions provided detailed maps of zinc concentrations, showcasing higher levels near the riverbank and gradual decreases inland. We also compared the performance of each interpolation method, noting the strengths and limitations of IDW, OK, and Universal Kriging in capturing spatial variations in zinc levels.
