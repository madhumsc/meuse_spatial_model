...
## Validation-Cross validation and RMSE.
```{r,warning=FALSE,message=FALSE}
...
```{r,warning=FALSE,message=FALSE}
rmseOK <- round(sqrt(mean(krig.ok.cv$residual^2)), 2)
rmseOK
```
# Regression Kriging

## Relationship identification
With below graph we can identified there is a liner relationship with NO2 with station_altitude.
```{r,warning=FALSE,message=FALSE}
plot( no2.sf$station_altitude, no2.sf$NO2)
```

## Fit Variogram
we plot the variogram for above identified pattern.
...
## Isotropy assumption
Below variogram matches in the directions of 0, 45, 90, and 135 degrees, and it's safe to assume that the variation is constant in all directions, then we can conclude that there is isotropy. Therefore, we can proceed with modeling using kriging.
```{r,warning=FALSE,message=FALSE}
...
## Kriging width Regression
As the model is isotropy and we apply that Regression and do the kriging for interpolation
```{r,warning=FALSE,message=FALSE}
v.m.rk <- fit.variogram(v.rk, vgm(psill=15, model = "Exp", range = 20000, nugget = 1))
plot(v1, v.m)
krig.ok.cv <- krige.cv(NO2~station_altitude, no2.sf,v.m.rk)

```

## Regression Kriging Validation
Here we got lesser RMSE than ordinary kriging. seem this interpolation more accureate than above kriging model.

RMSE in Regression Kriging:
```{r,warning=FALSE,message=FALSE}
rmseRK <- round(sqrt(mean(krig.ok.cv$residual^2)), 2)
rmseRK
```
...
It seems that both  Regression Kriging have relatively better than Ordinary Kriging as RMSE value is less in Regression Kriging.
Therefore, we can assume that Regression Kriging  methods good slitly better for spatial interpolation NO2 concentrations than Ordinary Kriging.