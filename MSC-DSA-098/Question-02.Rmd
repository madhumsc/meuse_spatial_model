...
For the core of our analysis, we employed three primary Spatial interpolation techniques: Inverse Distance Weighting (IDW), Ordinary Kriging (OK), and Regression Kriging. These methods, particularly OK and Regression Kriging, offer sophisticated approaches to spatial interpolation, considering both spatial autocorrelation and a deterministic trend. We fine-tuned these kriging models, fitting variogram models and validating their performance to ensure accurate predictions and validate it using RMSE.

The pinnacle of our study was the prediction and mapping of zinc concentrations across the Meuse river region using kriging and done coss validation with RMSE. Leveraging the power of kriging, we generated detailed maps depicting spatially continuous predictions of zinc levels. These maps not only provided insights into the spatial variability of zinc concentrations but also highlighted potential hotspots and areas of environmental concern along the river.

In conclusion, our study showcases the effectiveness of Spatial interpolation techniques in analyzing and predicting spatial variations in zinc concentrations and model validate with RMSE. By integrating advanced spatial analysis methods like kriging into our study and validate accuracy of our kriging model by finding RMSE, we were able to extract meaningful insights, contributing to a better understanding of environmental dynamics in the Meuse river region.
...
vgmRegression=variogram(log(zinc)~sqrt(dist), meuse)
vgmRegression.fit = fit.variogram(vgmRegression,  model = vgm(1, "Sph", 900, 1))

plot(vgmRegression,vgmRegression.fit )
```

## Prediction with Regression kriging
```{r,warning=FALSE,message=FALSE}
rKrig<- krige(log(zinc) ~ 1, meuse, meuse.grid, model = vgmRegression.fit )
rKrig_df <- data.frame(coordinates(meuse.grid), zinc_predicted = rKrig$var1.pred)
```
## Check isotropy
...
## Variogram map
With this, we can also say that the mean is distributed slightly equally in all directions.
```{r,warning=FALSE,message=FALSE}
...
ggplot(rKrig_df, aes(x = x, y = y, fill = zinc_predicted)) +
  geom_tile() +
  scale_fill_viridis() +
...
```{r,warning=FALSE,message=FALSE}
rKrig.okriging.cv <- krige.cv(log(zinc) ~ 1, meuse, model = vgmRegression.fit)

```
### RMSE in Regression Kriging
```{r,warning=FALSE,message=FALSE}
rmseRK <- round(sqrt(mean(rKrig.okriging.cv$residual^2)), 2)
rmseRK
```
...
With this, we can also say that the mean is distributed slightly equally in all directions.
```{r,warning=FALSE,message=FALSE}
vgm2.map <- variogram(log(zinc) ~ 1, meuse, cutoff = 1500, width = 100, map = TRUE)
plot(vgm2.map, threshold = 5)
```
...
```{r,warning=FALSE,message=FALSE}
rmseOK <- round(sqrt(mean(lzn.okriging.cv$residual^2)), 2)
rmseOK
```
...
Further more we have validate which kriging method more accurate based on RMSE(Root Mean square error). 

| Kriging  Type    | RMSE         |
|------------------|--------------|
|Regression Kriging| `r rmseRK`   |
|Ordinary Kriging  | `r rmseOK`   |

It seems that both Ordinary Kriging and Regression Kriging have nearly equal RMSE values. Therefore, we can assume that both methods are equally good for spatial interpolation zinc concentrations.